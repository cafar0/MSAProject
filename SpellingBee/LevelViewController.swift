...
class LevelViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    @IBAction func onNextButton(_ sender: Any) {
        ...
        viewController.levelModel = levelArray?[levelIndex!]
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if collectionView === self.collectionView {
            ...
            cell.letterLabel.text = levelModel?.incompleteWord[indexPath.row]
            return cell
        }
            
        else if collectionView === self.collectionView2 {
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: cellId, for: indexPath) as! LetterCollectionViewCell
            cell.letterLabel.text = levelModel?.options[indexPath.row]
            return cell
        }
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if collectionView === self.collectionView2 {
            ...
            if (levelModel?.completeWord.contains(cell.letterLabel.text!))!{
                
                let index = levelModel?.completeWord.index(of: cell.letterLabel.text!)
